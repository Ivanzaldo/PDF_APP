...
def window_merge():
    ...
    listbox.insert(0, *filename_list)
    ...
    def add_clicked():
        ...
        if error == False:
            ...
            if check_all_file.get() == 1:
                filename_dict[filename]=(0,count)
            else:
                filename_dict[filename]=(int(initial_page.get())-1,int(final_page.get()))
    ...
    def merge_clicked():
        ...
        if len(filename_list)<=1:
            ...
            merger = PdfMerger()
            for pdf,paginas in filename_dict.items():
...
def window_split():
    ...
    def add_clicked():
        ...
        if len(filename_list)<1 and filename == "":
            tk.messagebox.showerror(title="ERROR", message="No hay archivos seleccionados")
            error = True
        else:
            if error != True :
        ...
        if error == False:
            listbox.delete(0, tk.END)
            size_temp = len(filename_list)
            filename_list.append("Split"+str(size_temp+1)+".pdf "+"page "+initial_page.get()+" to page "+final_page.get())
            listbox.insert(0, *filename_list)
            filename_dict["Split"+str(size_temp+1)+".pdf "]=(int(initial_page.get())-1,int(final_page.get()))
    ...
    def split_clicked():
        ...
        if len(filename_list)<1:
            ...
            if (check_all_file.get() == 1):
                merger = PdfMerger()
                for pdf,paginas in filename_dict.items():
                    input_temp = open(filename, "rb")
                    merger.append(fileobj=filename, pages=paginas)
                    input_temp.close()
